...
function Header({ count, onNavigate, onCart, onMenu, menuOpen }) {
  return <><div className="announcement">MELT-FREE DELIVERY — THERMAL PACKAGING ON EVERY ORDER</div>
    <header className="header"><button className="brand" onClick={() => onNavigate('/')}>FORGED<span>CACAO</span></button>
      <nav className="nav" aria-label="Main navigation"><button onClick={() => onNavigate('/')}>Shop</button><button onClick={() => onNavigate('/wholesale')}>Wholesale</button><button onClick={() => document.getElementById('craft')?.scrollIntoView({ behavior: 'smooth' })}>Our craft</button></nav>
    ...
    {menuOpen && <div className="mobile-menu" role="navigation" aria-label="Mobile navigation"><button onClick={() => onNavigate('/')}>Shop all</button><button onClick={() => onNavigate('/wholesale')}>Wholesale gifts</button><button onClick={onCart}>Your bag ({count})</button></div>}</>;
}
...
export function App() {
  const [path, setPath] = useState(window.location.pathname); const [cart, setCart] = useState([]); const [cartOpen, setCartOpen] = useState(false); const [menuOpen, setMenuOpen] = useState(false);
  const navigate = (to) => { window.history.pushState({}, '', to); setPath(to); setMenuOpen(false); window.scrollTo(0,0); };
